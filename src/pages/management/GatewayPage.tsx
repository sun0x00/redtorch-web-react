...
export class GatewayPage extends React.Component<any> {
  ...
  public render() {
    ...
    },
    {
      ...
      data: 'boolean',
      onRender: (item) => {
        if (item.status === ConnectStatusEnum.DISCONNECTED) {
          return (
            <span style={{ color: 'red' }}>已经断开</span>
          );
        } else if (item.status === ConnectStatusEnum.CONNECTED) {
          return (
            <span style={{ color: 'green' }}>已经连接</span>
          );
        } else if (item.status === ConnectStatusEnum.DISCONNECTING) {
          return (
            <span style={{ color: 'orange' }}>正在断开</span>
          );
        } else if (item.status === ConnectStatusEnum.CONNECTING) {
          return (
            <span style={{ color: 'orange' }}>正在连接</span>
          );
        }
        return (
          <span style={{ color: 'aqua' }}>未知</span>
        );
      }
    }, {
      key: "gatewayName",
      ...
      data: 'string',
      onRender: (item) => {
        if (item.gatewayType === GatewayTypeEnum.TRADE_AND_MARKET_DATA) {
          return (
            <span>行情和交易</span>
          );
        } else if (item.gatewayType === GatewayTypeEnum.MARKET_DATA) {
          return (
            <span>行情</span>
          );
        } else if (item.gatewayType === GatewayTypeEnum.TRADE_ONLY) {
          return (
            <span>交易</span>
          );
        }
      ...
      data: 'number',
      onRender: (item) => {
        if (item.gatewayAdapterType === GatewayAdapterTypeEnum.CTP) {
          return (
            <span>CTP</span>
          );
        } else if (item.gatewayAdapterType === GatewayAdapterTypeEnum.IB) {
          return (
            <span>IB</span>
          );
        }
      ...
      onRender: (item) => {
        return (
          <span>
            {
              (item.status === ConnectStatusEnum.CONNECTED || item.status === ConnectStatusEnum.CONNECTING) ?
                <ActionButton
      ...
      <Stack>
        ...
        >
          <Stack gap={12} padding={20}>
            ...
            {this.state.editGatewayAdapterType === GatewayAdapterTypeEnum.CTP ?
              <Stack.Item align="center">
            ...
            {this.state.editGatewayAdapterType === GatewayAdapterTypeEnum.CTP ?
              <Stack.Item align="center">
                <Stack horizontal={true} tokens={{ childrenGap: 50 }} styles={{ root: { width: '100%' } }}>
                  <Stack {...columnProps}>
                    {(this.state.editGatewayType === GatewayTypeEnum.TRADE_AND_MARKET_DATA || this.state.editGatewayType === GatewayTypeEnum.MARKET_DATA) ?
                    ...
                    {(this.state.editGatewayType === GatewayTypeEnum.TRADE_AND_MARKET_DATA || this.state.editGatewayType === GatewayTypeEnum.TRADE_ONLY) ?
            ...
            {this.state.editGatewayAdapterType === GatewayAdapterTypeEnum.IB ?
              <Stack.Item align="center">
  ...
  private validateAllField() {
    ...
    // 验证CTP字段
    if (this.state.editGatewayAdapterType === GatewayAdapterTypeEnum.CTP) {
      ...
      if (this.state.editGatewayType === GatewayTypeEnum.TRADE_AND_MARKET_DATA || this.state.editGatewayType === GatewayTypeEnum.MARKET_DATA) {
      ...
      if (this.state.editGatewayType === GatewayTypeEnum.TRADE_AND_MARKET_DATA || this.state.editGatewayType === GatewayTypeEnum.TRADE_ONLY) {
